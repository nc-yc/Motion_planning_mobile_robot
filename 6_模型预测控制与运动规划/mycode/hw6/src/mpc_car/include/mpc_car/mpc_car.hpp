...
class MpcCar {
 private:
  ...
  void linearization(const double& phi,
    ...
    // ...
    Ad_ << 0, 0, -1 * v * sin(phi), cos(phi),
        0, 0, v * cos(phi), sin(phi),
        0, 0, 0, tan(delta) / ll_,
        0, 0, 0, 0;

    Ad_ = dt_ * Ad_ + MatrixA::Identity();
    ...
    for (int i = 0; i < tau; ++i) {
      ...
      if (i == 0) {
        ...
        if (false) {
          ...
          for (int j = 0; j <= i - 1; j++) {
            /*Multiplying A sequentially from right to left causes numerical instability when N_ is large.*/
            BB.block(n * i, m * (i - j - 1), n, m) = Ad_ * BB.block(n * i, m * (i - j), n, m);
          }
        }

        BB.block(i * n, i * m, n, m) = Bd_;
        for (int j = i - 1; j >= 0; --j) {
          /*Dynamic programming from top to bottom multiplies one A at a time without causing numerical instability*/
      ...
      //set U
      U.coeffRef(i * m + 0, 0) = historyInput_[tau - i - 1][0];
      U.coeffRef(i * m + 1, 0) = historyInput_[tau - i - 1][1];
    }

    // get new_x
    Eigen::MatrixXd new_x;
    new_x.setZero(n * tau, 1);
    new_x = BB * U + AA * x0 + gg;
    // get x0_delay , x0_delay=x(t+tau)
    x0_delay = new_x.block((tau - 1) * n, 0, n, 1);
    return x0_delay;
  }
 ...
 public:
  MpcCar(ros::NodeHandle& nh) : nh_(nh) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      // -ddelta_max <= ddelta=delta_i-delta_i_1 <= ddelta_max
      if (i != 0) {
        Cu_.coeffRef(i * 3 + 2, (i - 1) * m + 1) = -1;  // ddelta=delta_i-delta_i_1
      }
  ...
  int solveQP(const VectorX& x0_observe) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      // ...
      if (i < N_ - 1) {
        qx.coeffRef(i * n + 0, 0) = -1 * xy(0);
        qx.coeffRef(i * n + 1, 0) = -1 * xy(1);
        qx.coeffRef(i * n + 2, 0) = -1 * phi * rho_;
        qx.coeffRef(i * n + 3, 0) = 0;
      } else {
        qx.coeffRef(i * n + 0, 0) = -1 * xy(0) * rhoN_;
        qx.coeffRef(i * n + 1, 0) = -1 * xy(1) * rhoN_;
        qx.coeffRef(i * n + 2, 0) = -1 * phi * rho_ * rhoN_;
