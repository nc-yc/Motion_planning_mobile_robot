...
double Homeworktool::OptimalBVP(Eigen::Vector3d _start_position,Eigen::Vector3d _start_velocity,Eigen::Vector3d _target_position)
{
    ...
    */
    ...
    for(int i = 0; i < 4; i++){ // loop through four roots
        t=std::real(matrix_eigenvalues(i));
        img = std::imag(matrix_eigenvalues(i));
        if((t <= 0) || std::abs(img) >= 1e-16){
            continue;
        }
        for(int j = 0; j < 3; j++){ // loop through three dimensions
            a(j) = -12.0 / std::pow(t,3) * (_target_position(j) - _start_position(j) - _start_velocity(j) * t) + 6.0 / std::pow(t,2) * (-_start_velocity(j));
            b(j) = 6.0 / std::pow(_target_position(j) - _start_position(j) - _start_velocity(j) * t,2) - 2.0 / t *(-_start_velocity(j));
        }
        J = ( 1.0 / 3.0 * a.dot(a)) * std::pow(t,3) + a.dot(b) * std::pow(t,2)  + (b.dot(b) + 1) * t;
        if(optimal_cost<=J){//update min cost
            optimal_cost=J;
        }
    }
    return optimal_cost;
